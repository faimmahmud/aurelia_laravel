...
<?php ($pageTitle = 'Tour Packages | Aurelia Travel'); ?>
...
<section class="hero-shell" style="min-height:74vh;">
  <div class="hero-bg active" style="background-image:url('<?php echo e(asset('assets/package.jpg')); ?>'); opacity:1;"></div>
      ...
      <?php foreach ($packageData as $pkg): ?>
        <article class="package-card reveal" data-item="<?= e($pkg['category'] ?? 'all') ?>" data-search="<?= e($pkg['title'] . ' ' . ($pkg['country'] ?? '') . ' ' . ($pkg['description'] ?? '')) ?>">
          ...
          <div class="package-content">
            ...
            <?php
              $bookingUrl = route('booking.create', ['booking_type' => 'package', 'package_id' => $pkg['id'], 'package_name' => $pkg['title'], 'country' => $pkg['country'] ?? '', 'amount' => preg_replace('/[^0-9.]/', '', $pkg['price'] ?? '')]);
            ?>
            <div class="mt-2 d-flex flex-wrap gap-2">
              <a class="btn btn-gold px-4" href="<?= e($bookingUrl) ?>">Book now</a>
              <a class="btn btn-outline-dark px-4" href="<?= e(route('destinations')) ?>">See destinations</a>
            </div>
          </div>
        </article>
      <?php endforeach; ?>
...
<?php echo $__env->make('layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\faim\laravel-final-project\aurelia_laravel\resources\views/home/packages.blade.php ENDPATH**/ ?>